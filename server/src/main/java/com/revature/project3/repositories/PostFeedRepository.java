...
public interface PostFeedRepository extends JpaRepository<Post, Long> {
    ...
    public List<Post> findByDateBefore(LocalDateTime benchmark); //Get all the posts made BEFORE the given date

    @Query("select p from Post p where p.creationDate >= ?1")
    public List<Post> findByDateAfter(LocalDateTime benchmark); //Get all the posts made AFTER the given date
}
