...
const Posts = (props) => {
  ...
  useEffect(() => {
    let dateString = props.data.creationDate.slice(
      0,
      props.data.creationDate.indexOf("+")
    );
    let date = new Date(dateString);
    let seconds = 28800.95 - (date.getTime() - Date.now()) / 1000;
    console.log(seconds);
    if (seconds < 60) {
  ...
  return (
    <div>
              ...
              <div className="col-sm-3">
                ...
                <div className="dropdown-menu" aria-labelledby="navbarDropdown">
                  <a className="dropdown-item">
                    <span className="icon nav-link">Reaction 1</span>
                  </a>
                  <a className="dropdown-item">
                    <span className="icon nav-link">Reaction 2 ...</span>
                  </a>
                </div>
              </div>
              <div className="col-sm-3">
      ...
      {/*} <button className="btn btn-block" onClick={onPostCommentHandler}>Post</button> */}
      <br />
    </div>
  );
};
